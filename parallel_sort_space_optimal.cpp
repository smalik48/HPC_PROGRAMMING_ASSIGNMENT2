...
void parallel_sort(int * begin, int* end, MPI_Comm comm) {
  ...
  ***********************************************/
  ...
  int small = partition(begin, end ,pivot);
  int large = cur_local_size - small;
  ...
  compute_srcounts(sendcnts, recvcnts, barrier, small_arr, large_arr, new_size, comm, 1);
  ...
  MPI_Alltoallv(begin, &sendcnts[0], &sdispl[0], MPI_INT, &rbuf[0], &recvcnts[0], &rdispl[0], MPI_INT, comm);
  ...
  compute_srcounts(tmp_sendcnts, tmp_recvcnts, barrier, new_size, new_size, curr_size, comm, 0);
...
void compute_srcounts(vector<int> &sendcnts, vector<int> &recvcnts, int barrier, vector<int> small_arr, vector<int> large_arr, vector<int> new_size, MPI_Comm comm, int check){
  ...
  int j = 0 ;
  int k = 0 ;
  if(check==1) k = barrier;
  //send counts for small array
  for(int i = 0 ; i < p ; i++ ){
    //start filling the arrays
    int small_send = small_arr[i];
    while(small_send > 0){
      int temp = (small_send <= new_size[j])? small_send : new_size[j];
  ...
  if(check==1){
    //send counts for large array
    for(int i = 0 ; i < p ; i++ ){
      //start filling the arrays
      int large_send = large_arr[i];
      while(large_send > 0){
	int ltemp = (large_send <= new_size[k])? large_send : new_size[k];
	if(i == rank) sendcnts[k] = ltemp;
	if(k == rank) recvcnts[i] = ltemp;
	large_send -= ltemp;
	new_size[k] -= ltemp;
	if(new_size[k]==0)k++;
      }
